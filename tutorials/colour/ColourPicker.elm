...
changeColor x' y' m = let x = toFloat x' - 300
                          y =  ((toFloat y') - 300) * (-1)
                       in if radius (x,y) <= 100 then {m | h <- hue x y, s <- sat x y, cx <- x, cy <- y}
                          else if (x >= 140 && x <= 160) && (y<=100 && y>=(-100)) then {m | l <- light x y, sy <- y}
                          else m
...
update lastPos m = 
    case lastPos of
        ...
        Click clickdown -> m
        Touch tap -> changeColor tap.x tap.y m
...
display mod = let {-- VARIABLES NEEDED FOR DISPLAY FUNCTION --}
                  ...
                  hue' = toString <| round <| (if mod.h < 0 then ((mod.h+(2*pi))*180/pi)
                                              else (mod.h*180/pi) )
                  ...
                  -- additional images
                  highlight_background = toForm <| image 360 96 "images/highlight_background.png"
                  adjust_image = toForm <| image 101 143 "images/adjust_image.png"
                  select_image = toForm <| image 121 110 "images/select_image.png"
                  finalColour_image = toForm <| image 127 147 "images/finalColour_image.png"
                  highlight_image = toForm <| image 300 28 "images/highlight_image.png"
                  copy_marker1 = toForm <| image 98 32 "images/copy_marker1.png"
                  copy_marker2 = toForm <| image 119 33 "images/copy_marker2.png"
                  ...
                  colourWheel_image = toForm (image 204 204 "images/colourWheel_image.png")
                  lst2 = [0..100]
                  ...
                  selectorCircle = group 
                      ...
                      , traced (solid (hsl (mod.h+pi) mod.s 0.75)) <| segment (0,0) (mod.cx,mod.cy)
                      ]
...
type Update = Mouse (Int,Int) | Click Bool | Touch Position
...
combinedInput = Signal.merge lastPos (Signal.map Touch Touch.taps)
